...
def extended_euclid(a, b):
    ...
    b,a=max(a,b),min(a,b)
    el=[[b%a,1,b,-1*(b//a),a]] # put mod, values in a list
    while b%a>0: # comaprsion for while loop if greater than 0
        b,a=a,b%a # format
        el.append([b%a,1,b,-1*(b//a),a]) #break down values
    if len(el)>1: # if the length is greater than 1
        el.pop()
        el=el[::-1] 
        for i in range(1,len(el)): # loop through and assign
            el[i][1]*=el[i-1][3]
            el[i][3]*=el[i-1][3]
            el[i][3]+=el[i-1][1]

    expr=el[len(el)-1]
    strExpr="(" +str(el[0][0])+",[("+str(expr[3])+","+str(expr[4])+")"+","+"("+str(expr[1])+","+str(expr[2])+")])"
#kinda unsure how to format this properly so I broke it down as a list and strung together values.
